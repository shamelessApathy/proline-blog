...
(function($, _, window, document, undefined) {
	...
	ThemifyPageBuilder = {
		...
		bindEvents: function() {
			...
			if ( 'false' === themifyBuilder.isRevisionEnabled ) {
				$('.themify-builder-revision-dropdown-panel').css('visibility', 'hidden');
			}
		},
		...
		setColorPicker: function(context) {
			...
			.on('blur keyup', '.color_opacity', function(e) {
				var $input = $(this),
					tempOpacity = '',
					$minicolors = $input.parent().find('.builderColorSelect'),
					$field = $input.parent().find('.builderColorSelectInput');
		...
		optionsModule: function(event, isNewModule) {
			...
			// Hide browser title on preview icon
			self.hideTitlePreviewIcon();
		},
		...
		hideTitlePreviewIcon: function() {
			var previewIcon = $( '.tfl-icon' );
