...
class Themify_Builder {
	...
	public function async_load_builder_responsive(){
			$breakpoints = array();
                        ...
                        if( themify_builder_get( 'builder_responsive_design_tablet_landscape' ) ) {
                            $breakpoints['tablet_landscape'] = themify_builder_get( 'builder_responsive_design_tablet_landscape' );
			}
                        if( themify_builder_get( 'builder_responsive_design_tablet' ) ) {
                                $breakpoints['tablet'] = themify_builder_get( 'builder_responsive_design_tablet' );
			}
                        if( themify_builder_get( 'builder_responsive_design_mobile' ) ) {
                                $breakpoints['mobile'] = themify_builder_get( 'builder_responsive_design_mobile' );
			}
...
?>
	...
	function load_inline_js_script() {
        ...
        public static function getMapKey(){
            static $key = false;
            if($key===false){
                $key = themify_builder_get( 'builder_settings_google_map_key' );
            }
            return $key;
        }
	...
	function register_frontend_js_css() {
                wp_register_style( 'builder-styles', THEMIFY_BUILDER_URI . '/css/themify-builder-style.css',array(), THEMIFY_VERSION );
		...
		wp_localize_script('themify-main-script', 'themify_vars', array(
			...
			'isCached' => themify_builder_get('builder_disable_cache')
		));
		...
		wp_localize_script('themify-main-script', 'tbLocalScript', apply_filters('themify_builder_script_vars', array(
			...
			'animationInviewSelectors' => Themify_Builder_Model::is_premium()?array( '.module.wow', '.themify_builder_content .themify_builder_row.wow', '.module_row.wow', '.builder-posts-wrap > .post.wow' ):'',
	...
	/**
	 * Render element markup with ajax.
	 * 
	 * @access public
	 * @return json
	 */
	...
	public function get_builder_stylesheet( $builder_output ) {
		...
		if(!$builder_loaded && !self::is_front_builder_activate() && strpos($builder_output,'themify_builder_row')!==false){
			$builder_loaded = true;
			$url =  THEMIFY_BUILDER_URI . '/css/themify-builder-style.css';
			$handler = 'builder-styles';
			$style_tag = '<link rel="stylesheet" id="'.$handler.'" href="'.$url.'" media="all" />';
			$load_style = false;
			if(!is_admin() && !isset($_POST['action']) && !isset($_GET['builder_grid_activate']) && !TFCache::$style_iscreated && !TFCache::is_ajax() &&  TFCache::check_version() && TFCache::is_cache_activate()){
				$header = TFCache::$turnoff_cache && TFCache::check_builder_cache()?false:TFCache::create_scripts_dir('styles', 'header');
				if($header){
					global $wp_filesystem;
					$data = $wp_filesystem->get_contents( $header );
					if(strpos($data, $url)===false){
						wp_register_style($handler,$url, null, THEMIFY_VERSION);
						TFCache::$is_footer = false;
						TFCache::style_loader_tag($style_tag, $handler,$url);
						TFCache::$is_footer = 1;
						$load_style = true;
					}
				}
			...
			if( $load_style && wp_style_is($handler,'registered')){
				return $style_tag;
			}
		}
		return '';
	...
	function builder_admin_bar_menu($wp_admin_bar) {
		...
		if(!Themify_Builder_Model::is_premium()){
			$args[] = array(
				'id' => 'toggle_themify_builder_upgrade',
				'title' => sprintf('<span class="themify_builder_front_icon"></span> %s', __('Upgrade Builder', 'themify')),
				'parent' => 'themify_builder',
				'href' => '//themify.me/builder',
				'meta' => array('target' => '_blank', 'class' => '')
			);
		}
		foreach ($args as $arg) {
	...
	function get_custom_styling_legacy($style_id, $mod_name, $settings, $array = false, $format = 'tag') {
		...
		if ($mod_name == 'row') {
			...
			$styles_selector = array(
				...
				'.module_subrow h1' => array('color', 'font_family'),
				'.module_subrow h2' => array('color', 'font_family'),
				'.module_subrow h3:not(.module-title)' => array('color', 'font_family'),
				'.module_subrow h4' => array('color', 'font_family'),
				'.module_subrow h5' => array('color', 'font_family'),
				'.module_subrow h6' => array('color', 'font_family'),
				'.module_subrow>.builder_row_cover' => array('background_overlay'),
				'.module_subrow>.builder_row_cover:before' => array('background_overlay_hover'),
			);
	...
	public function get_template_column( $rows, $row, $cols, $col, $builder_id, $order_classes = array(), $echo = false, $frontedit_active = null ) {
		...
		if ( isset( $row['cols'] ) && 1 == count($row['cols']) ) {
	...
	public function get_template_sub_row( $rows, $cols, $modules, $mod, $builder_id, $echo = false, $frontedit_active = null ) {
		...
		array_push($dynamic_class,"module_subrow");
		
		if (isset($mod['styling']['background_repeat']) && !empty($mod['styling']['background_repeat'])) {
 			$dynamic_class[] = $mod['styling']['background_repeat'];
 		}
 		if (isset($mod['styling']['background_image']) && $mod['styling']['background_image'] && isset($mod['styling']['background_position']) && !empty($mod['styling']['background_position'])) {
 			$dynamic_class[] = 'bg-position-' . $mod['styling']['background_position'];
 		}
 		if(!empty($mod['styling']['background_type']) && $mod['styling']['background_type']==='image' && isset($mod['styling']['background_zoom']) && $mod['styling']['background_zoom']==='zoom' && $mod['styling']['background_repeat']=='repeat-none'){
 				$dynamic_class[] = 'themify-bg-zoom';
 		}
 		if (isset($mod['styling']['custom_css_column']) && $mod['styling']['custom_css_column']) {
 			$dynamic_class[] = $mod['styling']['custom_css_column'];
 		}
	...
	data-gutter="' . esc_attr($sub_row_gutter) . '"' : '';
		...
		echo sprintf('<div class="themify_builder_sub_row clearfix %s"%s %s %s>', esc_attr($print_sub_row_classes), $sub_row_attr, $sub_row_column_alignment_data, $video_data);
		?>
	...
	*/
	...
	function recursive_style_generator($data, $style_id, $sub_row = '') {
		...
		foreach ($data as $row_index => $row) {
			...
			if (!empty($row['styling']) && is_array($row['styling']) && empty($sub_row)) {
			...
			// Sub Row Style
			if(!empty($sub_row) && !empty($row['styling']) && is_array($row['styling'])){
				$selector2 = '.module_row_' . $sub_row[8] . ' .module_column_' . $sub_row[10] . ' .sub_row_' . $sub_row[8] . '-' . $sub_row[10] . '-' . $row['row_order'];
				$css_to_save .= $this->get_custom_styling(
								$selector2, "subrow", $row['styling'], false, 'css'
				);
				// responsive styling
	 ...
	 * @since 2.2.6
	 */
	function enqueue_fonts( $google_fonts ) {
