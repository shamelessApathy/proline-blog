...
class Attribution_Commerce_Woo extends Attribution_Commerce {
	...
	public function added_to_cart() {
		...
		if ( false !== ( $cookie = Attribution_Cookie::get_cookie( 'added_to_cart' ) ) ) {
	...
	public function removed_from_cart() {
		...
		if ( false !== ( $cookie = Attribution_Cookie::get_cookie( 'removed_from_cart' ) ) ) {
	...
	/**
	 * Adds product properties to analytics.identify() when the order is completed successfully.
	 *
	 * @since  1.0.0
	 * @access public
	 *
	 ...
	 *                        for each different platform, we use func_get_args().
	 *
	 * @return array Filtered array of name and properties for analytics.identify().
	 */
	public function identify_completed_order() {
		...
		if ( did_action( 'woocommerce_thankyou' ) ) {
			...
			/* Because gateways vary wildly in their usage of the status concept, we check for failure rather than success. */
			if ( 'failed' !== $order->status && $identify === false) {
                              $identify = array(
...
/**
 * Bootstrapper for the Attribution_Commerce_Woo class.
 *
 * @since  1.0.0
 */
function attribution_commerce_woo() {
	$commerce = new Attribution_Commerce_Woo();

	return $commerce->init();
}

add_action( 'plugins_loaded', 'attribution_commerce_woo', 100 );
